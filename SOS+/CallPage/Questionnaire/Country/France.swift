...
struct EmergencyFRQuestion: View {
    ...
    @ViewBuilder
    private func destinationView(for emergencyType: String) -> some View {
        switch emergencyType {
        case "Vehicle accident", "Fire":
            FRbNumberView()
        case "Illness or poison", "Lost or injured":
            EmergencyNumberView()
        case "Crime":
            FRaNumberView()
        default:
    ...
    var body: some View {
        VStack {
            ...
            NavigationLink(
                destination: destinationView(for: selectedEmergencyType),
                label: {
                    Text("Next")
                        .padding()
...
struct EmergencyFRQuestion_Previews: PreviewProvider {
