...
struct FRbNumberView: View {
    ...
    var body: some View {
        VStack {
            ...
            HStack {
                SMSEmergencyButton(emergencyNumber: "18")
                
                Button(action: {
                    // Action for calling the emergency number
                    if let url = URL(string: "tel://18"), UIApplication.shared.canOpenURL(url) {
...
struct FRbNumberView_Previews: PreviewProvider {
